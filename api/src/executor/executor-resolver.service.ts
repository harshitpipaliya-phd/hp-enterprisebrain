import type { Recommendation, ExecutorType, Executor } from '@hpbrain/database';

export interface ResolverPolicy {
  allowedExecutorClasses: ExecutorType[];
  trustLevels: Record<string, number>; // executorType -> min confidence required to use it
}

export interface ResolutionResult {
  executorType: ExecutorType;
  rationale: string;
  alternativesConsidered: Array<{ executorType: ExecutorType; rejectedBecause: string }>;
}

export interface MatchResult extends ResolutionResult {
  matchedExecutor: Executor | null;
  matchRationale: string;
}

export interface ExecutorDirectoryPort {
  findAvailable: (tenantId: string, executorType: ExecutorType, requiredCapability?: string) => Promise<Executor[]>;
}

const DEFAULT_POLICY: ResolverPolicy = {
...
export class ExecutorResolverService {
  constructor(private readonly directory?: ExecutorDirectoryPort) {}

  resolve(recommendation: Recommendation, policy: ResolverPolicy = DEFAULT_POLICY): ResolutionResult {
    const alternatives: Array<{ executorType: ExecutorType; rejectedBecause: string }> = [];
    const candidates: ExecutorType[] = ['ai_agent', 'hybrid', 'software', 'human'];
