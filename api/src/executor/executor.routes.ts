...
export function executorRoutes(repo = defaultRepo, resolver = new ExecutorResolverService(defaultRepo)): Router {
  const router = Router();
  ...
  router.post('/:tenantId/match', async (req: AuthenticatedRequest, res: Response) => {
    const parsed = matchSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: 'invalid_body', details: parsed.error.flatten() });
    const recommendation = await defaultRecommendationRepo.findById(req.params.tenantId, parsed.data.recommendationId);
    if (!recommendation) return res.status(404).json({ error: 'recommendation_not_found' });
