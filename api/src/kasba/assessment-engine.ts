import type { CapabilityProficiency, KasbaElement } from '@hpbrain/database';

export interface KasbaScores {
...
export interface GapFinding {
  dimension: 'knowledge' | 'ability' | 'skill' | 'behaviour' | 'attitude';
 ...
 */
export function computeKasbaScore(latest: CapabilityProficiency | null): KasbaScores {
  if (!latest) return { knowledge: null, ability: null, skill: null, behaviour: null, attitude: null, overall: null };
  const dims = [latest.knowledgeLevel, latest.abilityLevel, latest.skillLevel, latest.behaviourLevel, latest.attitudeLevel];
  const assessed = dims.filter((d): d is number => d != null);
...
export function computeCapabilityGap(
  latest: CapabilityProficiency | null,
  targets: { knowledge?: KasbaElement | null; ability?: KasbaElement | null; skill?: KasbaElement | null; behaviour?: KasbaElement | null; attitude?: KasbaElement | null }
): GapFinding[] {
  const findings: GapFinding[] = [];
  const dims: Array<[GapFinding['dimension'], number | null | undefined, number | null | undefined]> = [
  ...
  for (const [dimension, current, target] of dims) {
    ...
    if (gap > 0) findings.push({ dimension, currentLevel: current ?? null, targetLevel: target, gap });
  }
 ...
 */
export function computeCapabilityTrend(history: CapabilityProficiency[]): CapabilityTrend {
...
export interface IndividualScoreBreakdown {
...
export interface IndividualScore {
  score: number | null;
  breakdown: IndividualScoreBreakdown;
}
...
export function computeIndividualScore(input: {
  ...
  const parts = [capabilityScore, input.decisionApprovalRate, input.executionSuccessRate].filter((p): p is number => p != null);
