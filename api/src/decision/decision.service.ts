import type { Decision, CreateDecisionInput, Recommendation, Policy } from '@hpbrain/database';
import { eventBus, DecisionEvents } from '@hpbrain/events';
import { ExecutorResolverService, DEFAULT_POLICY, type ResolverPolicy } from '../executor/executor-resolver.service.js';
...
export interface RecommendationLookupAndUpdatePort {
...
export interface PolicyLookupPort {
  list: (tenantId: string, policyType?: 'business_rule') => Promise<Policy[]>;
}
...
export class DecisionService {
  constructor(
    ...
    private readonly recommendations: RecommendationLookupAndUpdatePort,
    private readonly resolver: ExecutorResolverService = new ExecutorResolverService(),
    private readonly policies?: PolicyLookupPort
  ) {}
   ...
   */
  async tryAutoApprove(tenantId: string, recommendationId: string): Promise<Decision | null> {
    if (!this.policies) return null;
    const recommendation = await this.recommendations.findById(tenantId, recommendationId);
    if (!recommendation || recommendation.status !== 'pending') return null;
    if (recommendation.category === 'opportunity') return null; // hard rule, no policy can override this
  ...
  async approve(input: ApproveInput): Promise<Decision> {
  ...
  async get(tenantId: string, id: string): Promise<Decision | null> {
   ...
   */
  async reject(input: { tenantId: string; recommendationId: string; decidedBy: string; rationale: string }): Promise<Decision> {
    const recommendation = await this.recommendations.findById(input.tenantId, input.recommendationId);
    if (!recommendation) throw new Error('recommendation_not_found');
