...
export function caseRoutes(service = new CaseService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId/signal/:signalId', async (req: AuthenticatedRequest, res: Response) => {
  ...
  router.patch('/:tenantId/:id/transition', async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (e.message === 'case_not_found') return res.status(404).json({ error: 'not_found' });
      if (e.message.startsWith('invalid_transition') || e.message === 'resolved_requires_hypothesis') return res.status(409).json({ error: e.message });
      throw e;
    }
  });
  ...
  router.get('/:tenantId/:id/evidence', async (req: AuthenticatedRequest, res: Response) => {
