...
const ROOT_CAUSE_FAMILIES = ['Capability', 'Capacity', 'Process', 'Information', 'Motivation', 'Coordination', 'External', 'Policy'] as const;
...
export function hypothesisRoutes(service = new HypothesisService(defaultHypothesisRepo, defaultCaseService)): Router {
  const router = Router();
  ...
  router.get('/:tenantId/case/:caseId', async (req: AuthenticatedRequest, res: Response) => {
    return res.json(await service.ledger(req.params.tenantId, req.params.caseId));
  });

  router.post('/:tenantId/case/:caseId/:id/reject', async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (e.message === 'hypothesis_not_found') return res.status(404).json({ error: 'not_found' });
      if (e.message === 'rejection_requires_reason') return res.status(400).json({ error: e.message });
      throw e;
    }
  });

  router.post('/:tenantId/case/:caseId/:id/support', async (req: AuthenticatedRequest, res: Response) => {
  ...
  router.post('/:tenantId/case/:caseId/:id/confirm', async (req: AuthenticatedRequest, res: Response) => {
    try {
