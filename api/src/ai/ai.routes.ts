...
export function aiRoutes(): Router {
  ...
  router.get('/executions/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    return res.json(await executions.list(req.params.tenantId));
  });

  router.post('/evidence/summarize', async (req: AuthenticatedRequest, res: Response) => {
    ...
    if ('error' in result) return res.status(result.providerConfigured ? 502 : 501).json(result);
    return res.json(result);
  });

  router.post('/decision/explain', async (req: AuthenticatedRequest, res: Response) => {
  ...
  router.post('/recommendation/improve', async (req: AuthenticatedRequest, res: Response) => {
