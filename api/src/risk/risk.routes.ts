...
import { RiskRepository, RISK_CATEGORIES, RISK_IMPACTS, RISK_STATUSES, DecisionRepository, EsoExecutionRepository, OutcomeRepository, ReasoningStepRepository, RecommendationRepository, SignalRepository } from '@hpbrain/database';
...
export function riskRoutes(service = new RiskService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    return res.json(await service.list(req.params.tenantId, req.query.status as any));
  });

  router.get('/:tenantId/decision/:decisionId', async (req: AuthenticatedRequest, res: Response) => {
  ...
  router.get('/:tenantId/:id', async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!risk) return res.status(404).json({ error: 'not_found' });
    return res.json(risk);
  });

  router.post('/:tenantId/:id/mitigate', async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (e.message === 'risk_not_found') return res.status(404).json({ error: 'not_found' });
      throw e;
    }
  });
   ...
   * separate lookups.
   */
  router.get('/:tenantId/interventions', async (req: AuthenticatedRequest, res: Response) => {
   ...
   * logic per role.
   */
  router.get('/:tenantId/interventions/person/:personId', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const scoped = await Promise.all(allRisks.map(async (risk) => {
      if (!risk.decisionId || !risk.recommendationId) return null;
      const rec = await recommendations.findById(tenantId, risk.recommendationId);
      if (!rec?.reasoningStepId) return null;
      const step = await reasoningSteps.findById(tenantId, rec.reasoningStepId);
      if (!step?.signalId) return null;
      const signal = await signals.findById(tenantId, step.signalId);
      if (signal?.relatedEntityType !== 'Person' || signal?.relatedEntityId !== personId) return null;
      return risk;
    }));
    const personRisks = scoped.filter((r): r is NonNullable<typeof r> => r !== null);
