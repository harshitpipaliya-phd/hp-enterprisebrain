...
export interface ValidationCheck {
  ...
  passed: boolean;
  details?: string;
  violations?: Array<{ label?: string; id?: string; issue?: string }>;
}

export class GraphValidationRepository extends BaseRepository {
  async checkNodeExists(label: string, id: string, tenantId: string): Promise<boolean> {
  ...
  async checkTenantIsolation(tenantId: string): Promise<{ passed: boolean; violations: number }> {
  ...
  async checkOrphanNodes(tenantId: string): Promise<{ passed: boolean; orphans: Array<{ label: string; id: string }> }> {
  ...
  async checkRelationshipIntegrity(tenantId: string): Promise<{ passed: boolean; broken: Array<{ from: string; to: string; type: string }> }> {
    const cypher = `
  ...
  async checkDuplicateNodes(label: string, tenantId: string): Promise<{ passed: boolean; duplicates: number }> {
    const cypher = `
  ...
  async getStats(tenantId: string): Promise<{ nodes: number; relationships: number }> {
