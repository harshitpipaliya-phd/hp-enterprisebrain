import { eventBus, OrganizationEvents, DepartmentEvents, PersonEvents, CapabilityEvents, SignalEvents, EvidenceEvents, ReasoningEvents, RecommendationEvents, DecisionEvents, OutcomeEvents, LearningEvents, ExecutorEvents, PolicyEvents, RiskEvents, MentalModelEvents, CaseEvents, HypothesisEvents, type DomainEvent } from '@hpbrain/events';
 ...
 */
export function flattenCapabilityForGraph(data: Record<string, unknown>): Record<string, unknown> {
  const flattenDimension = (dimension: string, el: unknown): Record<string, unknown> => {
...
export class GraphSyncRepository extends BaseRepository {
  async syncOrganization(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncDepartment(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncPerson(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncCapability(data: Record<string, unknown>): Promise<void> {
  ...
  async syncSignal(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncEvidence(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncReasoningStep(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncRecommendation(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncDecision(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncOutcome(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncLearning(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncExecutor(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncPolicy(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncRisk(data: Record<string, unknown>): Promise<void> {
    const cypher = `
   ...
   */
  async syncCapabilityAssignment(data: Record<string, unknown>): Promise<void> {
  ...
  async syncMentalModel(data: Record<string, unknown>): Promise<void> {
    const cypher = `
   ...
   */
  async syncCase(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async syncHypothesis(data: Record<string, unknown>): Promise<void> {
    const cypher = `
  ...
  async linkCaseEvidence(tenantId: string, caseId: string, evidenceId: string): Promise<void> {
    const cypher = `
  ...
  async getNodeCount(tenantId: string, label: string): Promise<number> {
  ...
  async getNodesByLabel(tenantId: string): Promise<Record<string, number>> {
  ...
  async getRelationshipsByType(tenantId: string): Promise<Record<string, number>> {
    const cypher = `MATCH (n {tenantId: $tenantId})-[r]->(m) RETURN type(r) as type, count(r) as count`;
    const { records } = await this.run(cypher, { tenantId });
    const result: Record<string, number> = {};
    for (const record of records) {
      result[record.type as string] = Number(record.count);
    }
    return result;
  }

  async findOrphanNodes(tenantId: string): Promise<Array<{ label: string; id: string }>> {
    const labels = ['Organization', 'Department', 'Person', 'Capability', 'Signal', 'Evidence', 'ReasoningStep', 'Recommendation', 'Decision', 'Outcome', 'Learning', 'Executor', 'Policy', 'Risk', 'MentalModel', 'Case', 'Hypothesis'];
    const orphans: Array<{ label: string; id: string }> = [];
    for (const label of labels) {
      const cypher = `MATCH (n:${label} {tenantId: $tenantId}) WHERE NOT (n)--() RETURN n.id as id, labels(n) as labels`;
      const { records } = await this.run(cypher, { tenantId });
      for (const record of records) {
        orphans.push({ label, id: record.id as string });
      }
    }
    return orphans;
  }

  async findTenantIsolationViolations(): Promise<number> {
...
export class GraphSyncService {
  ...
  private readonly retryQueue: Array<{ data: Record<string, unknown>; label: string; attempts: number }> = [];
  ...
  private async handleCaseEvidenceLink(event: DomainEvent): Promise<void> {
  ...
  private async handleAssignmentEvent(event: DomainEvent): Promise<void> {
    const session = this.sessionFactory(event.tenantId);
    try {
      const repo = new GraphSyncRepository(session);
      await repo.syncCapabilityAssignment({ ...(event.payload as Record<string, unknown>), tenantId: event.tenantId });
      this.status.push({
  ...
  private async handleEvent(event: DomainEvent, label: string, isArchive = false): Promise<void> {
    const session = this.sessionFactory(event.tenantId);
    try {
      const repo = new GraphSyncRepository(session);
      const data = event.payload as Record<string, unknown>;
      const entity = (data[Object.keys(data).find((k) => k.toLowerCase() === label.toLowerCase()) || 'entity'] as Record<string, unknown>) ?? data;

      if (isArchive) {
        await repo.archiveNode(label, event.entityId, event.tenantId);
      } else {
        const syncMethod = `sync${label}` as keyof GraphSyncRepository;
        await (repo[syncMethod] as (d: Record<string, unknown>) => Promise<void>)(entity);
      }
      ...
      this.retryQueue.push({ data: event.payload as Record<string, unknown>, label, attempts: 0 });
    } finally {
      await session.close();
    }
  }
  ...
  async retryFailed(): Promise<SyncStatus[]> {
    ...
    for (const item of this.retryQueue) {
      ...
      try {
        const repo = new GraphSyncRepository(session);
        const syncMethod = `sync${item.label}` as keyof GraphSyncRepository;
        await (repo[syncMethod] as (d: Record<string, unknown>) => Promise<void>)(item.data as Record<string, unknown>);
        results.push({
          tenantId: 'retry',
          entityType: item.label,
          entityId: String((item.data as any).id ?? 'unknown'),
          action: 'retry',
          status: 'success',
  ...
  getStatus(tenantId?: string): SyncStatus[] {
    if (!tenantId) return [...this.status];
