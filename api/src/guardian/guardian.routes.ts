...
export function guardianRoutes(): Router {
  const router = Router();
  ...
  router.get('/:tenantId/student/:studentPersonId', async (req: AuthenticatedRequest, res: Response) => {
    return res.json(await repo.listForStudent(req.params.tenantId, req.params.studentPersonId));
  });

  router.delete('/:tenantId/:id', async (req: AuthenticatedRequest, res: Response) => {
