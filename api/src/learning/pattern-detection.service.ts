...
export class PatternDetectionService {
  detect(learnings: Learning[], minOccurrences = 2): DetectedPattern[] {
    const termMap = new Map<string, { learningIds: Set<string>; confidenceSum: number; count: number }>();

    for (const learning of learnings) {
      if (!learning.reusable) continue; // only patterns proven to work should cluster
 ...
 */
export function suggestCapabilityGaps(patterns: DetectedPattern[], existingCapabilityNames: string[]): CapabilityGapSuggestion[] {
