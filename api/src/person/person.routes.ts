...
import type { PersonStatus } from './person.types.js';
...
export function personRoutes(service = new PersonService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const status = (req.query.status as string | undefined) as PersonStatus | undefined;
   ...
   */
  router.get('/:tenantId/:id/twin', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const individualScore = computeIndividualScore({
      capabilityOveralls: capabilityScores.map((c) => c.scores.overall).filter((s): s is number => s != null),
