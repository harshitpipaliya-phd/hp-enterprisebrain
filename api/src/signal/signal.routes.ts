...
export function signalRoutes(service = new SignalService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const signals = await service.list(
      req.params.tenantId,
  ...
  router.get('/:tenantId/:id/timeline', async (req: AuthenticatedRequest, res: Response) => {
    const signal = await service.get(req.params.tenantId, req.params.id);
    if (!signal) return res.status(404).json({ error: 'not_found' });
