...
import {
  DecisionRepository, RecommendationRepository, OutcomeRepository, RiskRepository, EvidenceRepository, MentalModelRepository, SignalRepository,
...
export function computeStatistics(input: {
  allDecisions: Array<{ status: string }>;
  allRecommendations: Array<{ status: string; category: string }>;
  allOutcomes: Array<{ result: string; confidence: number }>;
  allRisks: Array<{ category: string; score: number; status: string }>;
  allEvidence: Array<{ confidence: number }>;
}) {
...
export function computeDecisionIntelligence(input: {
  allDecisions: Array<{ status: string; executorType: string; recommendationId: string | null; createdDate: string }>;
  allRecommendations: Array<{ id: string; category: string; status: string; createdDate: string }>;
}) {
...
export function computeOrganizationalIntelligenceScore(input: {
  ...
  averageRiskScore: number; // 0-10 scale, per Risk Engine's own scoring (Sprint 4)
...
export function analyticsRoutes(): Router {
   ...
   */
  router.get('/:tenantId/executive-summary', async (req: AuthenticatedRequest, res: Response) => {
    const tenantId = req.params.tenantId;
    ...
    const organizationalKnowledge = allModels
      ...
      .map((m) => ({ domain: m.domain, confidence: m.confidence, reinforcementCount: m.reinforcementCount, patternCount: ((m.rules as any).patterns ?? []).length }));
   ...
   */
  router.get('/:tenantId/department/:departmentId', async (req: AuthenticatedRequest, res: Response) => {
    ...
    return res.json({
      ...
      bySource: allSignals.reduce((acc: Record<string, number>, s) => { acc[s.source] = (acc[s.source] ?? 0) + 1; return acc; }, {}),
      bySeverity: allSignals.reduce((acc: Record<string, number>, s) => { acc[s.severity] = (acc[s.severity] ?? 0) + 1; return acc; }, {}),
    });
  });

  router.get('/:tenantId/decision-intelligence', async (req: AuthenticatedRequest, res: Response) => {
    const tenantId = req.params.tenantId;
   ...
   */
  router.get('/:tenantId/decisions/export.csv', async (req: AuthenticatedRequest, res: Response) => {
    const tenantId = req.params.tenantId;
    ...
    const rows = allDecisions.map((d) =>
      [d.id, d.recommendationId ?? '', d.decidedBy, d.executorType, d.status, d.confidence, d.createdDate]
