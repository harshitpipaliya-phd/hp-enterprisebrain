...
import type { HealthStatus } from '../observability/observability.types.js';
...
export class HealthService {
  constructor(
    private readonly healthRepo = new HealthCheckRepository(),
  ) {}
   ...
   */
  private async recordBestEffort(checkName: string, status: string, details?: Record<string, unknown>, responseTime?: number): Promise<void> {
    try {
  ...
  async checkDatabase(): Promise<{ status: string; responseTime?: number; details?: Record<string, unknown> }> {
  ...
  async checkNeo4j(): Promise<{ status: string; responseTime?: number; details?: Record<string, unknown> }> {
    const start = Date.now();
    try {
  ...
  async checkEvents(): Promise<{ status: string; responseTime?: number; details?: Record<string, unknown> }> {
    const start = Date.now();
    try {
  ...
  async checkSystem(): Promise<{ status: string; details?: Record<string, unknown> }> {
    try {
  ...
  async getHealthStatus(): Promise<HealthStatus> {
    ...
    return {
      ...
      checks: checks as Record<string, { status: string; responseTime?: number; details?: Record<string, unknown> }>,
