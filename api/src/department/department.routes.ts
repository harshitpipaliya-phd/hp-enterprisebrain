...
import type { DepartmentStatus, DepartmentType } from './department.types.js';
...
export function departmentRoutes(service = new DepartmentService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const status = (req.query.status as string | undefined) as DepartmentStatus | undefined;
