...
export function evidenceRoutes(service = new EvidenceService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    const evidence = await service.list(req.params.tenantId, req.query.source as string | undefined);
