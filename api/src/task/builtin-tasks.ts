 ...
 */
export function buildDefaultRegistry(): TaskRegistry {
  ...
  const analyzeSignals: TaskDefinition = {
    ...
    run: async (tenantId) => {
      ...
      for (const s of all) bySeverity[s.severity] = (bySeverity[s.severity] ?? 0) + 1;
      return { count: all.length, bySeverity };
    },
  };

  const summarizeEvidence: TaskDefinition = {
  ...
  const detectPatterns: TaskDefinition = {
  ...
  const searchKnowledge: TaskDefinition = {
  ...
  const summarizeRisks: TaskDefinition = {
   ...
   * compliance judgment.
   */
  const complianceStatus: TaskDefinition = {
    ...
    run: async (tenantId) => {
      ...
      const summary = await Promise.all(frameworks.map(async (fw) => {
        ...
        for (const c of criteria) byStatus[c.status] = (byStatus[c.status] ?? 0) + 1;
