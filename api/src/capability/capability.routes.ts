...
import type { CapabilityStatus } from './capability.types.js';
...
export function capabilityRoutes(service = new CapabilityService(defaultRepo)): Router {
  const router = Router();
  ...
  router.get('/:tenantId', async (req: AuthenticatedRequest, res: Response) => {
    ...
    const status = (req.query.status as string | undefined) as CapabilityStatus | undefined;
  ...
  router.get('/:tenantId/:id', async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!capability) return res.status(404).json({ error: 'not_found' });
  ...
  router.get('/:tenantId/:id/versions', async (req: AuthenticatedRequest, res: Response) => {
  ...
  router.post('/:tenantId/:id/assign', async (req: AuthenticatedRequest, res: Response) => {
    const { targetType, targetId } = req.body as { targetType?: string; targetId?: string };
    if (!targetType || !targetId) return res.status(400).json({ error: 'target_type_and_id_required' });
    const assignment = await service.assign(req.params.tenantId, req.params.id, targetType, targetId, req.user!.id);
    return res.status(201).json(assignment);
  });

  router.delete('/:tenantId/:id/assign/:targetType/:targetId', async (req: AuthenticatedRequest, res: Response) => {
    await service.unassign(req.params.tenantId, req.params.id, req.params.targetType, req.params.targetId);
    return res.status(204).end();
  });

  router.get('/:tenantId/:id/assignments', async (req: AuthenticatedRequest, res: Response) => {
