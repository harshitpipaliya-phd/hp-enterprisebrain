...
export function decisionRoutes(service = new DecisionService(defaultRepo, defaultRecommendationRepo, undefined, defaultPolicyRepo)) {
    const router = Router();
    ...
    router.post('/', async (req, res) => {
        ...
        try {
            ...
            if (e.message === 'recommendation_not_found')
    ...
    router.post('/:tenantId/:recId/reject', async (req, res) => {
    ...
    router.post('/:tenantId/:recId/auto-approve-attempt', async (req, res) => {
        const decision = await service.tryAutoApprove(req.params.tenantId, req.params.recId);
        if (!decision)
            return res.json({ autoApproved: false });
