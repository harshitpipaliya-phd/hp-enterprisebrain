...
export function eventRoutes() {
    ...
    router.get('/:tenantId/what-changed', async (req, res) => {
    ...
    router.get('/:tenantId/entity/:entityType/:entityId/timeline', async (req, res) => {
    ...
    router.get('/', async (req, res) => {
        ...
        if (entityType && entityId && tenantId) {
            events = await eventStore.findByEntity(tenantId, entityType, entityId);
        }
        else if (type) {
            events = await eventStore.findByType(type, Number(limit));
        }
        else if (tenantId) {
            events = await eventStore.findByTenant(tenantId, Number(limit));
        }
        else if (status) {
