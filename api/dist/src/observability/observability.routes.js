...
export function observabilityRoutes() {
    ...
    router.get('/metrics/:tenantId', async (req, res) => {
        ...
        const metrics = await observability.getMetrics(req.params.tenantId, metricName);
        return res.json(metrics);
    });
    router.get('/metrics/:tenantId/:metricName/aggregates', async (req, res) => {
    ...
    router.get('/health/neo4j', async (_req, res) => {
        const result = await health.checkNeo4j();
        res.status(result.status === 'healthy' ? 200 : 503).json(result);
    });
    router.get('/health/events', async (_req, res) => {
