...
async function main() {
    ...
    try {
        ...
        await session.run(`MERGE (t:_GraphMigrationTracker {id: 'singleton'}) SET t.appliedMigrations = coalesce(t.appliedMigrations, [])`);
        const trackerResult = await session.run(`MATCH (t:_GraphMigrationTracker {id: 'singleton'}) RETURN t.appliedMigrations AS applied`);
        const applied = trackerResult.records[0]?.get('applied') ?? [];
        ...
        for (const file of files) {
            ...
            await session.run(`MATCH (t:_GraphMigrationTracker {id: 'singleton'}) SET t.appliedMigrations = t.appliedMigrations + $file`, { file });
        }
