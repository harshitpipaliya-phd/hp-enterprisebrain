...
export function policyRoutes(service = new PolicyService(defaultRepo)) {
    const router = Router();
    ...
    router.post('/', async (req, res) => {
        ...
        const policy = await service.create({ ...parsed.data, rules: parsed.data.rules, createdBy: req.user.id });
        return res.status(201).json(policy);
    });
    router.get('/:tenantId', async (req, res) => {
        return res.json(await service.list(req.params.tenantId, req.query.policyType));
    });
    ...
    router.get('/:tenantId/:id/history', async (req, res) => {
    ...
    router.post('/:tenantId/:id/version', async (req, res) => {
        ...
        if (!parsed.success)
            return res.status(400).json({ error: 'invalid_body', details: parsed.error.flatten() });
        try {
            const policy = await service.createVersion(req.params.tenantId, req.params.id, parsed.data.rules, req.user.id);
            return res.status(201).json(policy);
        }
        catch (e) {
            if (e.message === 'policy_not_found')
                return res.status(404).json({ error: 'not_found' });
            throw e;
        }
    });
    router.post('/:tenantId/:id/evaluate', async (req, res) => {
