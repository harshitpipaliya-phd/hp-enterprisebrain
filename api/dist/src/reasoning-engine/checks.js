 ...
 */
export function detectMissingEvidence(signals, evidenceSignalIds, graceDays = 3) {
    const now = Date.now();
    return signals
 ...
 */
export function detectDuplicateSignals(signals, windowHours = 24) {
 ...
 */
export function detectUnaddressedHighSeveritySignals(signals, signalIdsWithRecommendations, graceDays = 2) {
    const now = Date.now();
    return signals
