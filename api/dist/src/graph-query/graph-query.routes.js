...
export function graphQueryRoutes() {
    const router = Router();
    router.use(authMiddleware, requireRole('admin', 'org_admin', 'tenant_admin', 'signal_admin'));
    router.get('/:tenantId/entity/:label/:id', async (req, res) => {
        ...
        try {
            ...
            if (e.message?.startsWith('unknown_label'))
    ...
    router.get('/:tenantId/entity/:label/:id/related', async (req, res) => {
        const repo = new GraphQueryRepository(sessionFor(req.params.tenantId));
        try {
