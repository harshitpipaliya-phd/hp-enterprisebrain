...
export function esoRuntimeRoutes(service = new EsoRuntimeService(defaultRepo)) {
    const router = Router();
    ...
    router.get('/:tenantId/eso/:esoId', async (req, res) => {
    ...
    router.patch('/:tenantId/:id/transition', async (req, res) => {
        ...
        try {
            ...
            if (e.message === 'execution_not_found')
    ...
    router.post('/:tenantId/:id/rollback', async (req, res) => {
        try {
    ...
    router.post('/:tenantId/:id/evidence', async (req, res) => {
        ...
        if (typeof evidenceId !== 'string' || !evidenceId)
            return res.status(400).json({ error: 'evidenceId_required' });
    ...
    router.get('/:tenantId/:id/evaluate', async (req, res) => {
        ...
        const linkedEvidence = (await Promise.all(evidenceIds.map((eid) => evidenceRepo.findById(tenantId, eid)))).filter((e) => e !== null);
