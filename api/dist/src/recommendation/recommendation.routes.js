...
export function recommendationRoutes(service = new RecommendationService(defaultRepo, defaultReasoningRepo)) {
    const router = Router();
    ...
    router.post('/', async (req, res) => {
        ...
        try {
            ...
            if (e.message === 'reasoning_step_not_found')
                return res.status(404).json({ error: 'reasoning_step_not_found' });
            throw e;
        }
    });
    router.get('/:tenantId', async (req, res) => {
        const recs = await service.list(req.params.tenantId, req.query.status);
     ...
     */
    router.get('/:tenantId/:id/explain', async (req, res) => {
        ...
        const relevantPolicies = allPolicies.filter((p) => p.rules.some((r) => (r.field === 'recommendation.category' && r.value === rec.category) || r.conditions?.some((c) => c.field === 'recommendation.category' && c.value === rec.category)));
