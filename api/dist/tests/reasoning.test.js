...
function createMockRepo() {
    ...
    return {
        ...
        findBySignal: async (tenantId, signalId) => Object.values(store).filter((s) => s.tenantId === tenantId && s.signalId === signalId),
        findById: async (_t, id) => store[id] ?? null,
    };
}
function evidenceList(confidences) {
...
test('ReasoningService: no evidence yields low base confidence (Watch-tier)', async () => {
    const s = new ReasoningService(createMockRepo(), { findBySignal: async () => [] });
...
test('ReasoningService: internal-corroborated evidence raises confidence above the low-confidence threshold', async () => {
    const s = new ReasoningService(createMockRepo(), { findBySignal: async () => evidenceList([0.9, 0.8]) });
...
test('ReasoningService: confidence never exceeds 0.95', async () => {
    const s = new ReasoningService(createMockRepo(), { findBySignal: async () => evidenceList([1, 1, 1, 1, 1, 1, 1, 1]) });
...
test('ReasoningService.reason increments stepOrder per signal', async () => {
    ...
    const s = new ReasoningService(repo, { findBySignal: async () => [] });
