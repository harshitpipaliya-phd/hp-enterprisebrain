...
function createMockRepo() {
    ...
    return {
        ...
        list: async (tenantId, orgId, status) => Object.values(store).filter((s) => s.tenantId === tenantId && (!orgId || s.orgId === orgId) && (!status || s.status === status)),
        updateStatus: async (_tenantId, id, patch) => {
...
test('SignalService.detect creates a signal with default severity/confidence', async () => {
    const s = new SignalService(createMockRepo());
...
test('SignalService.list filters by tenant and status', async () => {
    ...
    const s = new SignalService(repo);
