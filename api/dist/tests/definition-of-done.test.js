...
test('Definition of Done: Case -> Evidence -> Reasoning -> Recommendation -> Decision -> Execution -> Outcome -> Learning -> Memory', async () => {
    ...
    const caseService = new CaseService({
        create: async (input) => {
            ...
            const c = { id: nextId('case'), tenantId: input.tenantId, signalId: input.signalId ?? null, title: input.title, description: input.description ?? null, status: 'open', resolvedHypothesisId: null, createdBy: input.createdBy, createdDate: now, updatedDate: now };
            db.cases[c.id] = c;
            return c;
        },
    ...
    const hypothesisService = new HypothesisService({
        propose: async (input) => {
            const h = { id: nextId('hyp'), tenantId: input.tenantId, caseId: input.caseId, statement: input.statement, rootCauseFamily: input.rootCauseFamily, confidence: input.confidence ?? 0.5, status: 'proposed', supportingEvidenceIds: input.supportingEvidenceIds ?? [], rejectedReason: null, proposedBy: input.proposedBy, createdDate: new Date().toISOString() };
            db.hypotheses[h.id] = h;
            return h;
        },
        recordOutcome: async (_t, _c, originalId, status, proposedBy) => {
            const original = db.hypotheses[originalId];
            const h = { ...original, id: nextId('hyp'), status, proposedBy, createdDate: new Date().toISOString() };
            db.hypotheses[h.id] = h;
            return h;
        },
    ...
    const reasoningService = new ReasoningService({
        create: async (input) => {
            const r = { id: nextId('rs'), tenantId: input.tenantId, caseId: input.caseId ?? null, signalId: input.signalId ?? null, mentalModelId: input.mentalModelId ?? null, stepOrder: input.stepOrder, description: input.description, confidenceScore: input.confidenceScore, createdBy: input.createdBy, createdDate: new Date().toISOString() };
    ...
    const learningService = new LearningService({ create: async (input) => { const l = { id: nextId('learn'), tenantId: input.tenantId, outcomeId: input.outcomeId ?? null, mentalModelId: input.mentalModelId ?? null, pattern: input.pattern, description: input.description ?? null, confidence: input.confidence ?? 0.5, reusable: input.reusable ?? true, createdBy: input.createdBy, createdDate: new Date().toISOString() }; db.learnings[l.id] = l; return l; },
        list: async (t) => Object.values(db.learnings).filter((l) => l.tenantId === t), findReusable: async () => [] }, { findById: async (_t, id) => db.outcomes[id] ?? null }, mentalModelService);
