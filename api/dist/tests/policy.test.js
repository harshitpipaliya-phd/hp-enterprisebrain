...
function createMockRepo() {
    ...
    return {
        ...
        createVersion: async (tenantId, previousId, rules, createdBy) => {
            const previous = store[previousId];
            if (!previous)
                throw new Error('policy_not_found');
            const id = `pol-${nextId++}`;
            const p = { ...previous, id, rules, version: previous.version + 1, previousVersionId: previousId, createdBy };
            store[id] = p;
            return p;
        },
        findById: async (_t, id) => store[id] ?? null,
        list: async (tenantId) => Object.values(store).filter((p) => p.tenantId === tenantId),
        history: async (tenantId, policyId) => {
            const chain = [];
            let current = store[policyId] ?? null;
            while (current) {
...
test('PolicyService.evaluate matches rules against a context object', async () => {
    const s = new PolicyService(createMockRepo());
