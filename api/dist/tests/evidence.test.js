...
function createMockRepo() {
    ...
    return {
        ...
        findBySignal: async (tenantId, signalId) => Object.values(store).filter((e) => e.tenantId === tenantId && e.signalId === signalId),
        list: async (tenantId) => Object.values(store).filter((e) => e.tenantId === tenantId),
    };
}
test('EvidenceService.collect stores evidence with provenance', async () => {
    const s = new EvidenceService(createMockRepo());
...
test('EvidenceService.forSignal returns only matching evidence', async () => {
    ...
    const s = new EvidenceService(repo);
