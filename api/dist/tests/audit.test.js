    ...
    return {
        append: async (log) => {
            ...
            if (log.correlationId)
                byCorrelation[log.correlationId] = byCorrelation[log.correlationId] || [];
            byCorrelation[log.correlationId]?.push(entry);
            if (log.eventId)
                byEvent[log.eventId] = entry;
            return entry;
        },
        findByEntity: async (tenantId, entityType, entityId) => store.filter(e => e.tenantId === tenantId && e.entityType === entityType && e.entityId === entityId),
        findByTenant: async (tenantId) => store.filter(e => e.tenantId === tenantId),
        findByCorrelationId: async (correlationId) => byCorrelation[correlationId] || [],
        findByEventId: async (eventId) => byEvent[eventId] ? [byEvent[eventId]] : [],
        search: async (tenantId, q) => store.filter(e => e.tenantId === tenantId && (e.action.includes(q) || e.entityType.includes(q) || e.actorName.includes(q))),
        count: async (tenantId) => tenantId ? store.filter(e => e.tenantId === tenantId).length : store.length,
        countByAction: async (tenantId) => {
            const counts = {};
            store.filter(e => e.tenantId === tenantId).forEach(e => { counts[e.action] = (counts[e.action] || 0) + 1; });
            return counts;
        },
        getActivityTimeline: async (tenantId, limit) => store.filter(e => e.tenantId === tenantId).slice(0, limit),
    };
}
