...
function createMockModelRepo() {
    ...
    return {
        ...
        findActiveByDomain: async (t, domain) => Object.values(store).find((m) => m.tenantId === t && m.domain === domain && m.status === 'active') ?? null,
        list: async (t) => Object.values(store).filter((m) => m.tenantId === t),
        reinforce: async (_t, id, pattern, newConfidence) => {
            const existing = store[id];
            const patterns = [...existing.rules.patterns, pattern];
...
test('MentalModelService.reinforceFromLearning creates the first model for a new domain', async () => {
    const s = new MentalModelService(createMockModelRepo());
    ...
    assert.deepEqual(model.rules.patterns, ['consultative bundling wins in mid-size territories']);
});
test('MentalModelService.reinforceFromLearning reinforces an existing domain model rather than creating a duplicate', async () => {
    ...
    assert.deepEqual(second.rules.patterns, ['pattern one', 'pattern two']);
});
...
test('LearningService.extract reinforces a mental model when reusable and a domain is given', async () => {
    ...
    const mentalModels = new MentalModelService(modelRepo);
    const learningStore = [];
    const learningRepo = {
        create: async (input) => { const l = { ...input, id: 'learn-1', createdDate: new Date().toISOString() }; learningStore.push(l); return l; },
        list: async () => learningStore,
        findReusable: async () => learningStore.filter((l) => l.reusable),
    };
    const s = new LearningService(learningRepo, { findById: async () => mockOutcome('success', 0.8) }, mentalModels);
...
test('LearningService.extract does not reinforce a mental model when the outcome failed', async () => {
    const mentalModels = new MentalModelService(createMockModelRepo());
    const learningRepo = {
        create: async (input) => ({ ...input, id: 'learn-1', createdDate: new Date().toISOString() }),
        list: async () => [],
        findReusable: async () => [],
    };
    const s = new LearningService(learningRepo, { findById: async () => mockOutcome('failure', 0.8) }, mentalModels);
