...
function fakeSession(tenantId, records = []) {
...
test('TenantService.create returns a tenant with id === tenantId', async () => {
    const svc = new TenantService(() => fakeSession('ignored', [
        {
...
test('TenantService.stats returns counts scoped by tenantId', async () => {
    const svc = new TenantService(() => fakeSession('tenant-xyz', [{ orgUnits: 2, people: 5, roles: 3, esos: 1 }]));
