...
function createMockRepo() {
    ...
    return {
        ...
        findByEso: async (tenantId, esoId) => Object.values(store).filter((e) => e.tenantId === tenantId && e.esoId === esoId),
        list: async (tenantId, status) => Object.values(store).filter((e) => e.tenantId === tenantId && (!status || e.status === status)),
...
test('EsoRuntimeService.execute queues an execution', async () => {
    const s = new EsoRuntimeService(createMockRepo());
