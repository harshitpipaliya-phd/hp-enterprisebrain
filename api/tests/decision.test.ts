...
import type { Recommendation, Decision, CreateDecisionInput } from '@hpbrain/database';
...
test('DecisionService.approve rejects unknown recommendation', async () => {
  const s = new DecisionService(createMockDecisionRepo() as any, { findById: async () => null, updateStatus: async () => null } as any);
