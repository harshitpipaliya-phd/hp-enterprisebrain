...
function createMockRepo() {
  const store: Record<string, Organization> = {};
  let nextId = 1;
  ...
  return {
    ...
    findById: async (_tenantId: string, id: string): Promise<Organization | null> => store[id] ?? null,
    list: async (tenantId: string): Promise<Organization[]> => Object.values(store).filter((o) => o.tenantId === tenantId),
    update: async (_tenantId: string, id: string, patch: UpdateOrganizationInput): Promise<Organization | null> => {
      const existing = store[id];
      if (!existing) return null;
      const updated = { ...existing, ...patch, updatedDate: new Date().toISOString() } as Organization;
      store[id] = updated;
      return updated;
    },
    archive: async (_tenantId: string, id: string): Promise<Organization | null> => {
      const existing = store[id];
      if (!existing) return null;
      const archived = { ...existing, status: 'archived', updatedDate: new Date().toISOString() } as Organization;
      store[id] = archived;
      return archived;
    },
  };
