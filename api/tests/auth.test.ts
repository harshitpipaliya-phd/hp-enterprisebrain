...
const svc = (rows: Record<string, Array<Record<string, unknown>>>) =>
  new AuthService((t: string) => fakeSession(t, rows[t] ?? []));
...
test('AuthService.changePassword rejects an incorrect current password', async () => {
  ...
  await assert.rejects(
    () => s.changePassword('t1', 'u1', 'wrong-password', 'new-password-123'),
    /invalid_current_password/
  );
});

test('AuthService.changePassword rejects a nonexistent user', async () => {
  const emptySession: any = { tenantId: 't1', run: async () => ({ records: [] }), close: async () => {} };
...
test('AuthService.changePassword succeeds with the correct current password and actually changes the hash', async () => {
  ...
  const session: any = {
    ...
    run: async (cypher: string, params: Record<string, unknown>) => {
      if (cypher.includes('SET')) { updatedHash = params.passwordHash as string; return { records: [] }; }
...
test('authMiddleware rejects missing token', async () => {
  ...
  const res = { status: (s: number) => { status = s; return res; }, json: () => {} } as any;
...
test('authMiddleware rejects invalid token', async () => {
  let status = 999;
  const req = { headers: { authorization: 'Bearer bad.token.here' } } as any;
  const res = { status: (s: number) => { status = s; return res; }, json: () => {} } as any;
  authMiddleware(req, res, () => {});
  assert.equal(status, 401);
});
