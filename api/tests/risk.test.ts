...
import type { Risk, CreateRiskInput } from '@hpbrain/database';

function createMockRepo() {
  const store: Record<string, Risk> = {};
  let nextId = 1;
  return {
    create: async (input: CreateRiskInput & { score: number }): Promise<Risk> => {
    ...
    findByDecision: async (t: string, decId: string) => Object.values(store).filter((r) => r.tenantId === t && r.decisionId === decId),
    list: async (t: string) => Object.values(store).filter((r) => r.tenantId === t),
    mitigate: async (_t: string, id: string, mitigation: string) => {
...
test('RiskService.computeScore is deterministic (same inputs, same output)', () => {
  const s = new RiskService(createMockRepo() as any);
